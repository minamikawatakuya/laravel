		...
		send(){
			...
			this.$axios.post(url,params).then(response => {
				...
				*/
				...
				var qa_length = response.data.length;
				var qa_num = "1";

				//alert(qa_num++);alert(qa_num);exit();

				this.$store.commit('update_qa_data',response.data);
				this.$store.commit('update_qa_length',qa_length);
				this.$store.commit('update_qa_num',qa_num);

				//alert(response.data.length);exit();

				window.location.href = 'hoge2'
